    ...
    S: ByteStream,
{
    ...
    pub fn decode_pixels_all(self) -> Result<Vec<Pixel>, DecodeAllError<S::IoError>> {
        let size = self
            .remaining
            .try_into()
    ...
    where
        F: Fn(Pixel) -> [u8; N],
    {
        let byte_size = usize::try_from(self.remaining)
            .ok()
            .and_then(|size| size.checked_mul(N))
            .ok_or(DecodeAllError::TooLarge)?;

        let mut buf = Vec::new();
        buf.try_reserve_exact(byte_size)
            .map_err(|_| DecodeAllError::TooLarge)?;

        for result in self {
            let pixel = result?;
            buf.extend(transform(pixel).into_iter());
        }
